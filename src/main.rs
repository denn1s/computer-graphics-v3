...
fn render_ui(framebuffer: &mut Framebuffer, texture_cache: &TextureManager) {
    let ui_width = 256;
    let ui_height = 256;
    let ui_x = ((framebuffer.width as f32 / 2.0) - (ui_width as f32 / 2.0)) as usize;
    let ui_y = (framebuffer.height - ui_height) as usize;

    for ty in 0..ui_height {
        for tx in 0..ui_width {
            let color = texture_cache.get_pixel_color('u', tx, ty);
            if color != TRANSPARENT_COLOR {
...
fn main() {
    ...
    while !window.window_should_close() {
        ...
        if mode == "2D" {
            ...
            render_ui(&mut framebuffer, &texture_cache);
        }
